...
async def Start_Command(message: types.Message):
    ...
    await message.answer("Hello Admin this are our commad to manage bot\n/add_vacancy - Add New Vacancy\n/add_company - Add Company\n/add_vacancy_type - Add Vacancy Type\n/add_requirement_tech - Add Required technologies", reply_markup=markup)
...
async def location_button_pressed(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    selected_locations = state_data.get("selected_locations", [])
    if location.name not in selected_locations:
        selected_locations.append(location.name)
    else:
        selected_locations.remove(location.name)  # Allow toggle off
    await state.update_data(selected_locations=selected_locations)

    # Update the keyboard to reflect the selection
    new_keyboard = await generate_location_keyboard(selected_locations)
    await callback_query.message.edit_reply_markup(reply_markup=new_keyboard)
    await callback_query.answer("Location updated.")


async def generate_location_keyboard(selected_locations=None):
    selected_locations = selected_locations or []
    locations = await sync_to_async(list)(Locations.objects.all())
    keyboard = types.InlineKeyboardMarkup()
    for location in locations:
        label = f"{location.name} {'✅' if location.name in selected_locations else ''}"
        callback_data = f'loc_{location.id}'
        keyboard.add(types.InlineKeyboardButton(label, callback_data=callback_data))
    keyboard.add(types.InlineKeyboardButton("Done", callback_data='loc_done'))
    return keyboard
...
async def vacancy_type_selected(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if data == "vtype_done":
        ...
        selected_vtypes = current_selection.get("vacancy_types", [])
        if vt_id in selected_vtypes:
            selected_vtypes.remove(vt_id)  # Toggle off
        else:
            selected_vtypes.append(vt_id)  # Toggle on
        await state.update_data(vacancy_types=selected_vtypes)
        new_keyboard = await generate_vacancy_type_keyboard(selected_vtypes)
        await callback_query.message.edit_reply_markup(reply_markup=new_keyboard)
        await callback_query.answer("Vacancy Type updated.")
...
async def requirement_tech_selected(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if data == "rtech_done":
        try:
            ...
            await state.finish()
            await callback_query.message.edit_text("Vacancy added successfully.")
        except Exception as e:
            await callback_query.message.edit_text(f"Error: {str(e)}")
    else:
        rt_id = int(data.split('_')[1])
        current_selection = await state.get_data()
        selected_rtechs = current_selection.get("requirement_techs", [])
        if rt_id in selected_rtechs:
            selected_rtechs.remove(rt_id)  # Toggle off
        else:
            selected_rtechs.append(rt_id)  # Toggle on
        await state.update_data(requirement_techs=selected_rtechs)
        new_keyboard = await generate_requirement_tech_keyboard(selected_rtechs)
        await callback_query.message.edit_reply_markup(reply_markup=new_keyboard)
        await callback_query.answer("Requirement Tech updated.")
...
async def generate_vacancy_type_keyboard(selected_vtypes=None):
    selected_vtypes = selected_vtypes or []
    vacancy_types = await sync_to_async(list)(VacancyType.objects.all())
    keyboard = InlineKeyboardMarkup()
    for vt in vacancy_types:
        label = f"{vt.name} {'✅' if vt.id in selected_vtypes else ''}"
        callback_data = f"vtype_{vt.id}"
...
async def generate_requirement_tech_keyboard(selected_rtechs=None):
    selected_rtechs = selected_rtechs or []
    requirement_techs = await sync_to_async(list)(RequirementTech.objects.all())
    keyboard = InlineKeyboardMarkup()
    for rt in requirement_techs:
        label = f"{rt.name} {'✅' if rt.id in selected_rtechs else ''}"
        callback_data = f"rtech_{rt.id}"
        keyboard.add(InlineKeyboardButton(label, callback_data=callback_data))
